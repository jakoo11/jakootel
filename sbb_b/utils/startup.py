...
LOGS = logging.getLogger("اعداد جاكو")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@sourcejakoo"))
    except BotMethodInvalidError:
        pass
    except ChannelsTooMuchError:
        LOGS.info("انضم بجروب السورس  اولا @sourcejakoo")
    except ChannelPrivateError:
        LOGS.critical(
            "تم حظرك من استخدام سورس جاكو  عليك الأعتذار الى مطور السورس @mvhmed"
        )
    try:
        await sbb_b(JoinChannelRequest("@mvhmed"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/1860b3cdf99b31e50dab5.jpg",
                caption="**تم تشغيل سورس جاكو بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "sourcejakoo"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "جروب بوت جاكو", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            ...
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info("لا توجد صلاحيات كافية لارسال الرسائل في جروب التخزين")
                if entity.default_banned_rights.invite_users:
                    LOGS.info("لا توجد صلاحيات كافية لاضافة الاعضاء في جروب التخزين")
        except ValueError:
            LOGS.error(
