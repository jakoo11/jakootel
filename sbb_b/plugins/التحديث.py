...
async def upstream(event):
    ...
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث لسورس جاكو"
        )
    try:
    ...
    if changelog == "" and not force_update:
        await event.edit("\n⌔∮ عزيز المستخدم انت تستخدم اخر اصدار من بوت جاكو 🫂♥")
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"ارسل `{cmdhd}تحديث الان` لتحديث سورس جاكو")

    if force_update:
        await event.edit("- يتم التحديث الاجباري لأخر اصدار من السورس انتظر قليلا")
    if conf == "الان":
        await event.edit("⌔∮ جارِ تحديث بوت جاكو يرجى الأنتظار قليلا")
...
async def upstream(event):
    if ENV:
        if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
            return await edit_or_reply(
                event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث جاكو"
            )
    elif os.path.exists("config.py"):
        return await edit_delete(
            event,
            f"⌔∮ يبدو انك نصبت جاكو عبر السيرفر يرجى ارسال `{cmdhd}تحديث الان`",
        )
    event = await edit_or_reply(event, "⌔∮ يتم الان الوصول الى معلومات التحديث انتظر")
    off_repo = "https://github.com/jakoo11/jakootel"
    os.chdir("/app")
    try:
        txt = "**⌔∮ عذرا لم يتم اكمال التحديث بسبب بعض الاخطاء " + "**اللوگ:**\n"
